...
if __name__ == "__main__":
    ...
    "Ozaboost M=10":ozaboost.OzaBoostClassifier,
    "Ozaboost M=50":ozaboost.OzaBoostClassifier,
    "Ozaboost M=100":ozaboost.OzaBoostClassifier
    }
    ...
    for (name, ensembler) in tqdm(algorithms.iteritems()):
        ...
        if name=="Greedy ozaboost":
            M = 1
        elif name=="Ozaboost M=10":
            M = 10
        elif name=="Ozaboost M=50":
            M = 50
        elif name=="Ozaboost M=100":
            M = 100
        ...
        for i in range(1,11):
            ...
            for t in range(T):
                ...
                y_cut = y_train[0:num]
                if name=="Greedy ozaboost":
                    model = test_dynamic.Test(ensembler, weak_learners[args.weak_learner], X_cut, y_cut, M)
                    train_accuracy, baseline_train_accuracy = model.test(weak_learners[args.weak_learner], X_cut, y_cut, X_test, y_test, M, trials=args.trials)
                    test_accuracy, baseline_test_accuracy = model.final_test(X_test, y_test, M)
                    
                else:
                    model = test.Test(ensembler, weak_learners[args.weak_learner], X_cut, y_cut, M)
                    train_accuracy, baseline_train_accuracy = model.test(X_cut, y_cut, X_test, y_test, M, trials=args.trials)
                    test_accuracy, baseline_test_accuracy = model.final_test(X_test, y_test, M)
