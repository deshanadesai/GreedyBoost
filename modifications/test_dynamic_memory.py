...
class Test():
    def __init__(self,algorithm, weak_learner, X, y, m, l):
        data = zip(X,y)
    ...
    def run_test(self, learner, data):
        ...
        for (X,Y) in data:
            ...
            if clf_no<100 and key!=Y and (conf<0.8 or conf==1.0):#  or (key==Y and conf<0.5):
                ...
                l = len(prev_points)
                if l>self.prev_points_size:
                    prev_x_pts = prev_points[l - self.prev_points_size:]
                else:
                    prev_x_pts = prev_points
                self.predictor.initialize_new_learner(learner, X,Y,weight, prev_x_pts)
            self.t += 1
